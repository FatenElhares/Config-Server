    ...
    node {
        ...
        try {
            ...
            stage('Create Docker Image') {
                ...
                def cmd = "ssh root@213.32.75.99 docker ps -a -q --filter ancestor=${config.projectName} --format='{{.ID}}'  | xargs docker stop 2>/dev/null | xargs docker rm -f"
                echo "if [[ ${cmd} -ne 0 ]]; then exit ${cmd}; fi "
            ...
            stage('Run Image') {
                ...
                sh 'ssh root@213.32.75.99  docker run -d -p 7072:7072 $config.projectName '
