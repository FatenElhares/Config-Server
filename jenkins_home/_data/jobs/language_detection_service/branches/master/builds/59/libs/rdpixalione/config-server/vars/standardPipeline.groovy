...
def call(body) {
    ...
    body()
    def RELEASE="ssh root@213.32.75.99  docker run -d -p 7072:7072 languagedetection --rm"
    ...
    node {
        ...
        try {
            ...
            stage('Create Docker Image') {
                ...
                sh 'if [[ $(ssh root@213.32.75.99 docker ps -a -q --filter ancestor=languagedetection --format="{{.ID}}"  | xargs docker stop 2>/dev/null | xargs docker rm -f) -ne 0 ]]; then exit $(ssh root@213.32.75.99 docker ps -a -q --filter ancestor=languagedetection --format="{{.ID}}"  | xargs docker stop 2>/dev/null | xargs docker rm -f); fi '
