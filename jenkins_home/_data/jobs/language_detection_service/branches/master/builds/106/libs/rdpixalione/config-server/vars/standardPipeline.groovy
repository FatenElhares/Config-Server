    ...
    node {
        ...
        try {
            ...
            stage('Create Docker Image') {
                ...
                def cmd=$/ ${ssh} docker ps -a -q --filter ancestor=${config.projectName} --format='{{.ID}}'  | xargs docker stop 2>/dev/null | xargs docker rm -f$/
                echo ${cmd}
                sh "if [[ ssh ${cmd} -ne 0 ]]; then exit ssh ${cmd}; fi "
