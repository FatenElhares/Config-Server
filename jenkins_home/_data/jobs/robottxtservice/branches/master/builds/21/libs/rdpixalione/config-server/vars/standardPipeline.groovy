    ...
    node {
        ...
        try {
            ...
            stage('Publish & Tag Image : Dev') {
                ...
                sh "docker tag ${config.projectName} ${config.projectName}:${config.Tag}Dev "
            ...
            stage('Push Image To Dev ') {
                ...
                sh "docker save ${config.projectName}:${config.Tag}Dev | bzip2 | pv | ssh ${ssh} 'bunzip2 | docker load' "
            ...
            stage('Deploy Image to Dev') {
                ...
                sh "ssh ${ssh}  docker run -d -p ${config.serverDomain}:${config.serverDomain} ${config.projectName}:${config.Tag}Dev"
            ...
            stage('Publish & Tag Image : Pre-Prod') {
                ...
                sh "docker tag ${config.projectName} ${config.projectName}:${config.Tag}PP "
                //envoyer vers Nexus 3
            ...
            stage('Push Image To Pre-Prod ') {
                ...
                sh "docker save ${config.projectName}:${config.Tag}PP | bzip2 | pv | ssh ${preprodssh} 'bunzip2 | docker load' "
                echo "Stage Test and Build Complete"
            }
            ...
            stage('Deploy Image To PreProd') {
                ...
                sh "ssh ${preprodssh}  docker run -d -p ${config.serverDomain}:${config.serverDomain} ${config.projectName}:${config.Tag}PP"
                echo "Realising is Done ! ..."
            }
