    ...
    node {
        ...
        try {
            ...
            stage('Cleanup Pre-Prod Env') {
                echo "Destroying the build environment..."
                def cmd = "ssh ${preprodssh}  docker images -f dangling=true -q | xargs docker rmi"
                try {
                    "${cmd}".execute()
                    echo "Cleaning ENV : successfully"
                }catch (Exception exception){
            ...
            stage('Deploy Image To PreProd') {
                ...
                try {
                    sh "ssh ${preprodssh} docker ps -a -q --filter ancestor=${config.projectName} --format=\'{{.ID}}\'  | xargs docker stop 2>/dev/null | xargs docker rm -f"
                    echo "Delete Container successfully"
                }catch (Exception exception){
                    echo "No Container to delete"
                }

                sh "ssh ${preprodssh}  docker run -d -p ${config.serverDomain}:${config.serverDomain} ${config.projectName}"
                echo "Realising is Done ! ..."
            }
