    ...
    node {
        ...
        try {
            ...
            stage('Clean Env') {
                ...
                sh "if [[ $(ssh ${ssh}  docker images -f dangling=true -q | xargs docker rmi) -ne 0 ]]; then exit $(ssh ${ssh}  docker images -f dangling=true -q | xargs docker rmi); fi "
                echo "Clean Complete"
            }

            stage('Run Image') {
                echo " Starting Release phase..."
                sh "ssh ${ssh}  docker run -d -p 7072:7072 $config.projectName "
